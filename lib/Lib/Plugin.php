...
abstract class Plugin implements PluginInterface {
	...
	public function plugin_action_links( $links ) {
		...
		if ( $this->has_premium() && ! $this->is_premium_active() ) {
			...
			$links['go_pro'] = sprintf( '<a href="%1$s" target="_blank" style="color: #39b54a; font-weight: bold;">%2$s</a>', esc_url( $pro_link ), esc_html__( 'Go Pro', 'wc-wholesale-manager' ) );
		}
	...
	*/
	...
	public function get_meta_links() {
		...
		if ( ! empty( $this->get_docs_url() ) ) {
			$links['docs'] = array(
				'label' => __( 'Documentation', 'wc-wholesale-manager' ),
		...
		if ( ! empty( $this->get_support_url() ) ) {
			$links['support'] = array(
				'label' => __( 'Support', 'wc-wholesale-manager' ),
		...
		if ( ! empty( $this->get_review_url() ) ) {
			$links['review'] = array(
				'label' => __( 'Review', 'wc-wholesale-manager' ),
				'url'   => $this->get_review_url(),
			);
		}

		$links['plugins'] = array(
			'label' => __( 'More Plugins', 'wc-wholesale-manager' ),
	...
	public function get_action_links() {
		...
		if ( ! empty( $this->get_settings_url() ) ) {
			$links['settings'] = array(
				'label' => __( 'Settings', 'wc-wholesale-manager' ),
